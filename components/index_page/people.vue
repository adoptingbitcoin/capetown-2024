          ...
          <div v-for="team in teamData" class="">
              <div class="cursor-pointer w-full border-b py-5 grid grid-cols-2 md:grid-cols-4 px-5 hover:text-black hover:bg-adoptingYellow" @click="uncollapsed[team.name] = !uncollapsed[team.name]">
                  ...
                  <div class="text-end px-5">
                      <client-only>
                          <div v-if="!team.isOpen"><font-awesome-icon :icon="['fa', 'plus']" /></div>
                          <div v-if="team.isOpen"><font-awesome-icon :icon="['fa', 'minus']" /></div>
                      </client-only>
                  </div>
              ...
              <Collapse :when="uncollapsed[team.name]" class="pb-36 vue-collapse">
                  <div class="grid grid-cols-1 md:grid-cols-5 gap-12 mb-5 mt-12 ">
